...
export default function StudentProfile(){
  const {id}=useParams(); const [student,setStudent]=useState<Student|null>(null); const [attendance,setAttendance]=useState<Page<AttendanceRecord>|null>(null); const [hifz,setHifz]=useState<Page<HifzProgress>|null>(null); const [error,setError]=useState('');
  ...
  return <><PageHeader eyebrow="Student records" title="Student Profile" description="Review identity, relationships, attendance, learning progress, and notes." actions={<Link className="btn" to={`/students/${id}/edit`}><Edit size={17}/> Edit student</Link>}/><div className="profile-hero"><ProtectedProfileImage endpoint={`/students/${student.id}/profile-image/`} hasImage={student.has_profile_image} name={student.full_name} size="lg"/><div><h1>{student.full_name}</h1><p>{student.student_id} · {student.gender}</p></div><span className="profile-status"><Badge tone={student.status}>{student.status}</Badge></span></div><div className="profile-grid">
    ...
    <Card><h2>Enrollment</h2><dl><dt>Enrollment date</dt><dd>{student.enrollment_date}</dd><dt>Dedicated teacher</dt><dd>{student.teacher?.name||'Unassigned'}</dd><dt>Teacher phone</dt><dd>{student.teacher?.phone||'—'}</dd><dt>Teacher email</dt><dd>{student.teacher?.email||'—'}</dd></dl></Card>
    <Card><h2>Primary parent / guardian</h2><dl><dt>Name</dt><dd>{student.parent?.name||'Unassigned'}</dd><dt>Relationship</dt><dd>{student.guardian_relationship}</dd><dt>Phone</dt><dd>{student.parent?.phone||'—'}</dd><dt>Email</dt><dd>{student.parent?.email||'—'}</dd></dl></Card>
